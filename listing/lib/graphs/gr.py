...
for n in n_values:
    ...
    plt.xlabel('$m$', fontsize=font_size_axis)
    plt.ylabel('$k$', fontsize=font_size_axis)
    plt.title(f'Generating Triples $\\{{r_k, r_m, r_n\\}}$ for $n={n}$', fontsize=font_size_title)
    ...
    if len(xticks) > 0 and len(yticks) > 0:
        plt.xticks(xticks, np.arange(2, n, tick_step), fontsize=font_size_axis)
        plt.yticks(yticks, np.arange(2, n, tick_step), fontsize=font_size_axis)
    else:
        plt.xticks([])
        plt.yticks([])
    ...
    plt.savefig(os.path.join(output_dir, f'n_{n:03d}.pdf'), dpi=300, bbox_inches='tight')
    plt.close()
